    ...
    def forward(
        ...
        if prompt_embedding is not None:
            if concat_prompt:  # concat prompt embedding with embedding_output
                ...
                extended_prompt_embedding = torch.zeros_like(embedding_output)
                extended_prompt_embedding[prompt_mask, :] = prompt_embedding.reshape(-1, prompt_dim)
                extended_prompt_embedding = extended_prompt_embedding.reshape(bsz, -1, prompt_dim)
                extended_prompt_embedding = extended_prompt_embedding.to(embedding_output.device)
                
                
                # pre_padding = torch.zeros(bsz, input_shape[1] - n_prompt_tokens - 1, prompt_dim).to(embedding_output.device)
                # mask_padding = torch.zeros(bsz, 1, prompt_dim).to(embedding_output.device)
                # extended_prompt_embedding = torch.cat([pre_padding, prompt_embedding, mask_padding], dim=1)                

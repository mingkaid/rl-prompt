...
def run_model(args):
    ...
    train_path = Path(os.path.join("../../data/16-shot", args.task, f'16-{args.seed}', 'train.tsv'))
    if args.perturbed:
        train_dataset = utils.load_augmented_trigger_dataset(train_path, templatizer, limit=args.limit)
    else:
        train_dataset = utils.load_trigger_dataset(train_path, templatizer, use_ctx=args.use_ctx, limit=args.limit)
    train_loader = DataLoader(train_dataset, batch_size=args.bsz, shuffle=True, collate_fn=collator)

    dev_path = Path(os.path.join("../../data/16-shot", args.task, f'16-{args.seed}', 'dev.tsv'))
    if args.perturbed:
        dev_dataset = utils.load_augmented_trigger_dataset(dev_path, templatizer)
    else:
        dev_dataset = utils.load_trigger_dataset(dev_path, templatizer, use_ctx=args.use_ctx)
    dev_loader = DataLoader(dev_dataset, batch_size=args.eval_size, shuffle=False, collate_fn=collator)
    
    test_path = Path(os.path.join("../../data/16-shot", args.task, f'16-{args.seed}', 'test.tsv'))
    if args.perturbed:
        test_dataset = utils.load_augmented_trigger_dataset(test_path, templatizer)
    else:
        test_dataset = utils.load_trigger_dataset(test_path, templatizer, use_ctx=args.use_ctx)
    ...
    logger.info(f'Dataset {args.task} Seed {args.seed}')
    ...
    for model_inputs, labels in test_loader:
        ...
        numerator += evaluation_fn(predict_logits, labels).sum().item()
        denominator += labels.size(0)
    test_metric = numerator / (denominator + 1e-13)
    
    os.makedirs(os.path.join('result', args.task), exist_ok=True)
    with open(os.path.join('result', args.task, f'result-{args.seed}.txt'), 'a') as f:
        f.write(f'Test Metrics: {test_metric}\n')

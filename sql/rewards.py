...
class GPT2SentimentBLEUNoInputReward(object):
    ...
    def __init__(
        ...
        dataset_clf_basepath = "/data/mingkai/prompt-generation/soft-Q-learning-for-text-generation/experiments/yelp_sentiment_classifier"
        ...
        self.lower = 'lower' in self.dataset
    ...
    def _load_tst_inputs(self) -> Dict[Tuple[Any], List[str]]: 
        ...
        if self.dataset == 'yelp':
            ...
            if self.dataset in ['shakespeare'] and self.seed in [1, 2, 3, 4]: 
                max_length = 25
...
#         input_length = max([len(sent) for sent in self._generator.tokenizer(source_strings)['input_ids']]) + 10
...
class PLMClassifierReward(object):
    ...
    def _get_few_shot_inputs(self, mode: str, target_labels: List[str], kshot: int, PAIR: bool): 
        ...
        if self._task_name in ['SST-2', 'yelp-2', 'mr', 'cr']:
            ...
            world_data = self._tst_inputs[(mode, 'LABEL_0')]
            sports_data = self._tst_inputs[(mode, 'LABEL_1')]
            business_data = self._tst_inputs[(mode, 'LABEL_2')]
            technology_data = self._tst_inputs[(mode, 'LABEL_3')]
        elif self._task_name in ['sst-5', 'yelp-5']:
            data_1 = self._tst_inputs[(mode, 'LABEL_0')]
            data_2 = self._tst_inputs[(mode, 'LABEL_1')]
            data_3 = self._tst_inputs[(mode, 'LABEL_2')]
            data_4 = self._tst_inputs[(mode, 'LABEL_3')]
            data_5 = self._tst_inputs[(mode, 'LABEL_4')]
            ...
            if idx == 0:
                if PAIR == True:
                    ...
                    if self._task_name == 'agnews':
                        random.Random(0).shuffle(self._tst_inputs[(mode, 'LABEL_2')])
                        random.Random(0).shuffle(self._tst_inputs[(mode, 'LABEL_3')])
                    elif self._task_name in ['sst-5', 'yelp-5']:
                        random.Random(0).shuffle(self._tst_inputs[(mode, 'LABEL_2')])
                        random.Random(0).shuffle(self._tst_inputs[(mode, 'LABEL_3')])
                        random.Random(0).shuffle(self._tst_inputs[(mode, 'LABEL_4')])
                else:
    ...
    def _load_few_shot_examples(self, task_name: str = 'SST-2', shot: int = 16, random_seed: int = 0):
        ...
        if shot in [24, 32, 64]:
            train_tsv = '/emnlp-2022-code/'+str(shot)+'-a-train.tsv'
            dev_tsv = '/emnlp-2022-code/'+str(shot)+'-a-dev.tsv'
        if task_name in ['SST-2', 'yelp-2', 'mr', 'cr']:
